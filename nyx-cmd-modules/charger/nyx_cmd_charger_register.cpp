...
static string descriptionStr = "Registers to device charger status/state changes and invokes callback.";
int cb_fired = 0;
...
nyx_device_callback_function_t status_cb(nyx_device_handle_t device, void *ctx)
{
	...
	if (NYX_ERROR_NONE == error)
	{
		...
		cout << "Charger_charger_max_current=\"" << chargerStatus.charger_max_current << "\"" << endl;
